const server = 'http://127.0.0.1:3000';

export const LOGIN = server+'/admin/login';
export const LOGOUT = server+'/admin/logout';
export const ADMIN = server+'/admin/count';
export const USER = server+'/admin/users';
export const ORDER = server+'/admin/order/';
export const CATEGORY_ADD = server+'/category';
export const CATEGORY_EDIT = server+'/category/edit';
export const CATEGORY_ORDER = server+'/category/order';
export const PRODUCT_UPLOADIMAGE = server+'/product/uploadImage';
export const PRODUCT_UPLOADETAILIMAGE = server+'/product/uploadetailImage';
export const PRODUCT_ADD = server+'/product';
export const PRODUCT_ORDER = server+'/product/order';
export const PRODUCT_STATUS = server+'/product/status';
export const PRODUCT_EDIT = server+'/product/edit';
export const PRODUCT_SEARCH = server+'/product/search';
export const ORDER_SEARCH = server+'/order/search';